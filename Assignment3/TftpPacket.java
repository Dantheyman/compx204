...
class TftpPacket
{
     ...
     */
    static public DatagramPacket createDATA(InetAddress dst, int port,  int block, byte[] data, int length)
    {
        /*
	 ...
	 * null.
	 */
	if (block<1||block>255||dst==null||port < 1 || port > 65535||length<1||length>512)
    {
        return null; 
    }
	...
	payload[0] = 2;
    payload[1] = (byte)block;
    System.arraycopy(data, 0, payload, 2,length);
    return new DatagramPacket(payload,length+2, dst, port);
    }
	 ...
	 * valid DatagramPacket, so return null.
	 */
	if (error==null||dst==null||port<1||port>65535)
    {
        return null; 
    }
	...
	if(data[0] == 1) {
        ...
        System.arraycopy(data, 2, bytes, 0, (length)-2);
        p.data=bytes; 
        return p;
	}
	else if(data[0] == 3) {
        ...
        p.type=Type.ERROR;
        p.block=Byte.toUnsignedInt(data[1]);
        byte[] error = new byte[data.length-1];
        System.arraycopy(data, 1, error, 0, error.length);
